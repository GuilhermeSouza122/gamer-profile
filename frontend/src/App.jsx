...
const formatUnlockTime = (epochSeconds) => epochSeconds ? new Date(epochSeconds * 1000).toLocaleString('pt-BR', { dateStyle: 'medium', timeStyle: 'short' }) : null
...
function App() {
  ...
  const [siteAchievementDetails, setSiteAchievementDetails] = useState([])
  ...
  useEffect(() => {
    if (siteAchievementsOpen || profileViewOpen) {
      apiFetch(`${API}/site-achievements`).then((response) => response.ok ? response.json() : []).then(setSiteAchievementDetails).catch(() => setSiteAchievementDetails([]))
      apiFetch(`${API}/profile/me`).then((response) => response.ok ? response.json() : null).then((data) => { if (data) setProfile(data) }).catch(() => {})
    }
  }, [siteAchievementsOpen, profileViewOpen])
  ...
  const levelBaseXp = profile ? (profile.level - 1) * 1000 : 0
  const levelProgress = profile ? Math.min(100, Math.round(((profile.xpPoints - levelBaseXp) / 1000) * 100)) : 0

  if (showLogin) return <LoginScreen />

  return <main className="app-shell">
    <header className="topbar"><div className="brand">{profile?.avatarUrl ? <img className="steam-avatar" src={profile.avatarUrl} alt="Avatar da Steam" /> : <span className="brand-mark">GP</span>}<span>{profile?.displayName || 'GAMER PROFILE'}</span></div><div className="topbar-actions"><div className="status"><span className="pulse" /> STEAM CONNECTED</div><button className="site-achievements-button" onClick={() => { setProfileViewOpen(false); setSiteAchievementsOpen(!siteAchievementsOpen) }}>{siteAchievementsOpen ? 'VOLTAR À BIBLIOTECA' : 'CONQUISTAS DO SITE'}</button><button className="site-achievements-button" onClick={() => { setSiteAchievementsOpen(false); setProfileViewOpen(!profileViewOpen) }}>{profileViewOpen ? 'VOLTAR À BIBLIOTECA' : 'MEU PERFIL'}</button><button className="sync-button" onClick={synchronizeSteam} disabled={syncing}>{syncing ? 'SINCRONIZANDO...' : 'SINCRONIZAR STEAM'}</button></div></header>
    {!profileViewOpen && !siteAchievementsOpen && <><section className="hero"><div><p className="eyebrow">GAMER PROFILE</p><h1>Catálogo de<br /><em>troféus.</em></h1><p className="hero-copy">Sua coleção de jogos, conquistas e progresso em um só lugar.</p>{profile && <div className="level-card"><div className="level-heading"><strong>LVL {profile.level}</strong><span>{profile.xpPoints} / {profile.level * 1000} XP</span></div><div className="level-track"><div style={{ width: `${levelProgress}%` }} /></div><small>{Math.max(0, profile.level * 1000 - profile.xpPoints)} XP para o próximo nível</small></div>}</div></section><section className="summary-grid"><div><strong>{summary?.totalGames ?? '—'}</strong><span>JOGOS TOTAIS</span></div><div><strong>{summary ? Math.round(summary.totalPlaytimeMinutes / 60) : '—'}</strong><span>HORAS JOGADAS</span></div><div><strong>{summary?.unlockedAchievements ?? '—'}</strong><span>CONQUISTAS DESBLOQUEADAS</span></div><div><strong>{summary ? `${summary.completionPercentage}%` : '—'}</strong><span>CONCLUSÃO GERAL</span></div></section></>}
    {error && <div className="notice">{error} <span>Verifique se o backend está rodando em localhost:8080.</span></div>}
    {syncMessage && <div className="sync-message">{syncMessage}</div>}
    {profileViewOpen ? <section className="profile-page"><div className="profile-identity">{profile?.avatarUrl ? <img src={profile.avatarUrl} alt="Avatar da Steam" /> : <span>GP</span>}<div><p className="eyebrow">STEAM PROFILE</p><h2>{profile?.displayName || 'Seu perfil'}</h2><p>{profile?.username || ''}</p></div></div><div className="profile-level-card"><div className="level-heading"><strong>NÍVEL {profile?.level ?? 1}</strong><span>{profile?.xpPoints ?? 0} / {(profile?.level ?? 1) * 1000} XP</span></div><div className="level-track"><div style={{ width: `${levelProgress}%` }} /></div><small>{Math.max(0, (profile?.level ?? 1) * 1000 - (profile?.xpPoints ?? 0))} XP para o próximo nível</small></div><div className="profile-stat-grid"><div><strong>{siteAchievementDetails.filter((item) => item.unlocked).length}</strong><span>CONQUISTAS DO SITE</span></div><div><strong>{summary?.unlockedAchievements ?? '—'}</strong><span>CONQUISTAS STEAM</span></div><div><strong>{summary?.totalGames ?? '—'}</strong><span>JOGOS</span></div><div><strong>{summary ? `${Math.round(summary.totalPlaytimeMinutes / 60)}h` : '—'}</strong><span>TEMPO JOGADO</span></div><div><strong>{summary ? `${summary.completionPercentage}%` : '—'}</strong><span>CONCLUSÃO GERAL</span></div></div><div className="profile-section-title"><p className="eyebrow">PROGRESSO</p><h3>Conquistas desbloqueadas</h3></div><div className="profile-achievement-list">{siteAchievementDetails.filter((item) => item.unlocked).map((item) => <div key={item.code}><span>✦</span><strong>{item.name}</strong><small>+{item.xp} XP</small></div>)}{!siteAchievementDetails.some((item) => item.unlocked) && <p className="muted">Nenhuma conquista própria desbloqueada ainda.</p>}</div></section> : siteAchievementsOpen ? <section className="site-achievements"><p className="eyebrow">GAMER PROFILE / ORIGINAL TROPHIES</p><h2>Conquistas do site</h2><p className="site-achievements-copy">Conquistas próprias do Gamer Profile, independentes das conquistas importadas das plataformas.</p><div className="site-achievement-grid">{SITE_ACHIEVEMENTS.map(([name, description, criterion, icon]) => { const detail = siteAchievementDetails.find((item) => item.code === SITE_CODES[name]); const unlocked = detail?.unlocked; return <article className={unlocked ? 'site-achievement unlocked' : 'site-achievement'} key={name}><span>{icon}</span><h3>{name}</h3><p>{description}</p><strong className="site-achievement-xp">+{detail?.xp ?? '—'} XP</strong><small>{unlocked ? `DESBLOQUEADA${detail.unlockedAt ? ` · ${new Date(detail.unlockedAt).toLocaleDateString('pt-BR')}` : ''}` : criterion.toUpperCase()}</small></article> })}</div></section> : <section className="content-grid">
